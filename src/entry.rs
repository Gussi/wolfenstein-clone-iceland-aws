...
async fn load_textures(assets: &BrowserAssets) -> Result<TextureSet, String> {
    ...
    let pan_png = assets.load_bytes("assets/textures/pan.png").await.map_err(|e| e.to_string())?;
    ...
    let pan_tiles = decode_sprite_sheet(&pan_png, 128)?;
